...
module.exports = async function (taskDetails, days = 7) {
	...
	const result = await taskDetails.query(`
		SELECT
			cla_user.id,
			cla_user.email,
			(NOT ('{${emailNotificationCategory.userNotUnlockedBook}}'::TEXT[] <@ cla_user.email_opt_out)) AS should_receive_email
		FROM cla_user
		LEFT JOIN asset_school_info
			ON cla_user.id = asset_school_info.user_id
		LEFT JOIN user_not_unlocked_email_send_log
			ON cla_user.id = user_not_unlocked_email_send_log.user_id
			AND user_not_unlocked_email_send_log.days >= ${days}
		WHERE (cla_user.date_created_initial_password + interval '${days} days' <= NOW())
			AND asset_school_info.user_id IS NULL
			AND user_not_unlocked_email_send_log.user_id IS NULL
	...
	if (result && result.rowCount > 0) {
		...
		const tableName = "user_not_unlocked_email_send_log";
