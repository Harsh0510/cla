const sendEmailByDaysRaw = require("../../../../core/auth/async_task/user_not_created_copies_email_sender/sendEmailByDays");
let mockResultData, mockData;
let isGetUsersData = false;
let isInsertIntoLogTable = false;
let isCalledEmailSender = false;
jest.mock(`../../../../core/auth/async_task/user_not_created_copies_email_sender/emailSender`, () => {
...
test(`Function render correctly for 14 days`, async () => {
	mockData = 14;
	expect(await sendEmailByDays(mockData)).toEqual(null);
	expect(isGetUsersData).toEqual(true);
	expect(isInsertIntoLogTable).toEqual(true);
...
test(`Not send an email but store the log into log table when user disabled "Receive email reminders to unlock my first book"`, async () => {
	mockData = 7;
	mockResultData = {
		rowCount: 1,
		rows: [
