const userCreateRaw = require("../../core/auth/user-create.js");
...
test("no school-id provided when user role as cla-admin", async () => {
...
test("invalid school-id provided when user role as cla-admin", async () => {
	ctx.sessionData.user_role = "cla-admin";
	const params = getParams();
...
test("invalid school-id as -35 provided when user role as cla-admin", async () => {
	ctx.sessionData.user_role = "cla-admin";
	const params = getParams();
	params.school_id = -35;
...
test("invalid school-id provided when user role as school-admin", async () => {
	ctx.sessionData.user_role = "school-admin";
...
test("invalid school-id as -125 provided when user role as school-admin", async () => {
	ctx.sessionData.user_role = "school-admin";
...
test('search institution with no results should return "Institution not found"', async () => {
	ctx.appDbQuery = async function (query, values) {
		const queryText = query.replace(/[\s\t\n\r]+/g, " ").trim();
		if (queryText.indexOf("cla_role") !== -1) {
			return { rows: [{ count: 1 }] };
