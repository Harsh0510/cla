const publisherUpdateRaw = require("../../core/admin/publisher-update");
const Context = require("../common/Context");

let ctx;
let mockIsIncludeModifyUserIdForPublisher;
let mockIsIncludeDateEditedForPublisher;
...
function resetAll() {
	...
	mockIsIncludeModifyUserIdForPublisher = false;
	mockIsIncludeDateEditedForPublisher = false;
}
...
async function publisherUpdate(data) {
	let err = null;
	try {
		ctx.body = await publisherUpdateRaw(data, ctx);
	} catch (e) {
		err = e;
	}
	return err;
...
test(`Error "Unauthorized" when user role is teacher`, async () => {
	...
	expect(await publisherUpdate(data)).toEqual(new Error("401 ::: Unauthorized"));
});
...
test(`Error "ID invalid" when id is not pass`, async () => {
	...
	const data = getValidRequest();
	delete data.id;
	expect(await publisherUpdate(data)).toEqual(new Error("400 ::: ID invalid"));
});
...
test(`Error "ID must not be negative" when id is negative`, async () => {
	...
	const data = getValidRequest();
	data.id = -20;
	expect(await publisherUpdate(data)).toEqual(new Error("400 ::: ID must not be negative"));
});
...
test(`Error "No fields changed" when not pass the active and buy_book_rules`, async () => {
	...
	delete data.temp_unlock_opt_in;
	expect(await publisherUpdate(data)).toEqual(new Error("400 ::: No fields changed"));
});
...
test(`Error "'Buy Book' rules must be an array" when buy_book_rules invalid`, async () => {
	...
	expect(await publisherUpdate(data)).toEqual(new Error(`400 ::: 'Buy Book' rules must be an array`));
});
...
test(`Error "Too many 'Buy Book' rules" when buy_book_rules length > 10`, async () => {
	...
	expect(await publisherUpdate(data)).toEqual(new Error(`400 ::: Too many 'Buy Book' rules`));
});
...
test(`Error "Too many 'Buy Book' rules" when buy_book_rules some values are invalid`, async () => {
	...
	expect(await publisherUpdate(data)).toEqual(new Error(`400 ::: Rule 7 not a string`));
});
...
test(`Unexpected SQL error`, async () => {
	...
	ctx.doAppQuery = (query, values) => {
		...
		if (query.indexOf("UPDATE publisher") == 0) {
			throw new Error("Unknown error");
		}
	};
	const data = getValidRequest();
	expect(await publisherUpdate(data)).toEqual(new Error("Unknown error"));
});
...
test(`Success`, async () => {
	...
	const data = getValidRequest();
	expect(await publisherUpdate(data)).toBeNull();
	expect(ctx.body).toMatchObject({ result: true });
});

test("Error when temp_unlock_opt_in is not a boolean value", async () => {
	const data = getValidRequest();
	ctx.sessionData.user_role = "cla-admin";
	data.temp_unlock_opt_in = "";
	expect(await publisherUpdate(data)).toEqual(new Error("400 ::: Temp Unlock Opt-in should be a boolean"));
});
...
test(`Ensure modified_by_user_id and date_edited updated successfully in database`, async () => {
	...
	ctx.doAppQuery = (query, values) => {
		query = query.trim().replace(/\s+/g, " ");
		if (query.indexOf("UPDATE publisher") == 0) {
			mockIsIncludeModifyUserIdForPublisher = query.indexOf("modified_by_user_id") !== -1 ? true : false;
			mockIsIncludeDateEditedForPublisher = query.indexOf("date_edited") !== -1 ? true : false;
			return { rows: [], rowCount: 1 };
		}
	...
	expect(await publisherUpdate(data)).toBeNull();
	expect(ctx.body).toMatchObject({ result: true });
	expect(mockIsIncludeModifyUserIdForPublisher).toEqual(true);
	expect(mockIsIncludeDateEditedForPublisher).toEqual(true);
