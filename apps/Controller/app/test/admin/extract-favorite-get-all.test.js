const extractFavoriteGetAllRaw = require("../../core/admin/extract-favorite-get-all");
...
async function extractFavoriteGetAll(data) {
	let err = null;
	try {
		ctx.body = await extractFavoriteGetAllRaw(data, ctx);
	} catch (e) {
		err = e;
	}
	return err;
...
test(`Error when not login`, async () => {
	...
	expect(await extractFavoriteGetAll(data)).toEqual(new Error("401 ::: Unauthorized"));
});
...
test(`Error when limit is invalid`, async () => {
	...
	const data = getValidRequest();
	data.limit = "test";
	expect(await extractFavoriteGetAll(data)).toEqual(new Error("400 ::: Limit invalid"));
});
...
test(`Error when limit is negative`, async () => {
	...
	const data = getValidRequest();
	data.limit = -40;
	expect(await extractFavoriteGetAll(data)).toEqual(new Error("400 ::: Limit must be positive"));
});
...
test(`Error when offset is invalid`, async () => {
	...
	const data = getValidRequest();
	data.offset = "test";
	expect(await extractFavoriteGetAll(data)).toEqual(new Error("400 ::: Offset invalid"));
});
...
test(`Error when offset is negative`, async () => {
	...
	const data = getValidRequest();
	data.offset = -40;
	expect(await extractFavoriteGetAll(data)).toEqual(new Error("400 ::: Offset must not be negative"));
});
...
test(`Error when sort_direction not pass`, async () => {
	...
	expect(await extractFavoriteGetAll(data)).toEqual(new Error("400 ::: Sort Direction not provided"));
});
...
test(`Error when sort_direction is positive integer number`, async () => {
	...
	expect(await extractFavoriteGetAll(data)).toEqual(new Error("400 ::: Sort Direction invalid"));
});
...
test(`Error when sort_field not pass`, async () => {
	...
	expect(await extractFavoriteGetAll(data)).toEqual(new Error("400 ::: Sort field not found"));
});
...
test(`Error when query is invalid`, async () => {
	...
	const data = getValidRequest();
	data.query = 123;
	expect(await extractFavoriteGetAll(data)).toEqual(new Error("400 ::: Query invalid"));
});
...
test(`Unknown Error while executing the count query`, async () => {
	...
	const data = getValidRequest();
	expect(await extractFavoriteGetAll(data)).toEqual(new Error("401 ::: Must be associated with an institution"));
});
...
test(`Success while executing the result query`, async () => {
	...
	const data = getValidRequest();
	expect(await extractFavoriteGetAll(data)).toBeNull();
...
test(`Success when sort by publisher`, async () => {
	ctx.sessionData.user_role = "school teacher";
	ctx.sessionData.school_id = 1;
	ctx.doAppQuery = (query, values) => {
		query = query.trim().replace(/\s+/g, " ");
	...
	delete data.offset;
	data.sort_direction = "D";
	data.sort_field = "publisher";
	expect(await extractFavoriteGetAll(data)).toBeNull();
	expect(ctx.body).toMatchObject({ data: [], unfiltered_count: 5 });
});

test(`Success when sort by is_unlocked`, async () => {
	ctx.sessionData.user_role = "school teacher";
	ctx.sessionData.school_id = 1;
	ctx.doAppQuery = (query, values) => {
		query = query.trim().replace(/\s+/g, " ");
	...
	delete data.offset;
	data.sort_direction = "D";
	data.sort_field = "is_unlocked";
	expect(await extractFavoriteGetAll(data)).toBeNull();
	expect(ctx.body).toMatchObject({ data: [], unfiltered_count: 5 });
});

test(`Success when sort by pdf_isbn13`, async () => {
	ctx.sessionData.user_role = "school teacher";
	ctx.sessionData.school_id = 1;
	ctx.doAppQuery = (query, values) => {
		query = query.trim().replace(/\s+/g, " ");
	...
	delete data.offset;
	data.sort_direction = "D";
	data.sort_field = "pdf_isbn13";
	expect(await extractFavoriteGetAll(data)).toBeNull();
	expect(ctx.body).toMatchObject({ data: [], unfiltered_count: 5 });
});

test(`Success when sort by publication_year`, async () => {
	ctx.sessionData.user_role = "school teacher";
	ctx.sessionData.school_id = 1;
	ctx.doAppQuery = (query, values) => {
		query = query.trim().replace(/\s+/g, " ");
	...
	delete data.offset;
	data.sort_direction = "D";
	data.sort_field = "publication_year";
	expect(await extractFavoriteGetAll(data)).toBeNull();
	expect(ctx.body).toMatchObject({ data: [], unfiltered_count: 5 });
});

test(`Success when sort by edition`, async () => {
	ctx.sessionData.user_role = "school teacher";
	ctx.sessionData.school_id = 1;
	ctx.doAppQuery = (query, values) => {
		query = query.trim().replace(/\s+/g, " ");
	...
	delete data.offset;
	data.sort_direction = "D";
	data.sort_field = "edition";
	expect(await extractFavoriteGetAll(data)).toBeNull();
	expect(ctx.body).toMatchObject({ data: [], unfiltered_count: 5 });
});

test(`Success when sort by date_created`, async () => {
	ctx.sessionData.user_role = "school teacher";
	ctx.sessionData.school_id = 1;
	ctx.doAppQuery = (query, values) => {
		query = query.trim().replace(/\s+/g, " ");
