const approvedDomainGetAllRaw = require("../../core/admin/approved-domain-get-all");
...
async function approvedDomainGetAll(data) {
	let err = null;
	try {
		ctx.body = await approvedDomainGetAllRaw(data, ctx);
	} catch (e) {
		err = e;
	}
	return err;
...
test(`Error when user is school-admin`, async () => {
	...
	expect(await approvedDomainGetAll(data)).toEqual(new Error("401 ::: Unauthorized"));
});
...
test(`Error when sort_field not pass`, async () => {
	...
	expect(await approvedDomainGetAll(data)).toEqual(new Error("400 ::: Sort field not found"));
});

test(`Error when sortDirection is wrong pass`, async () => {
	const data = getValidRequest();
	data.sort_direction = "C";
	expect(await approvedDomainGetAll(data)).toEqual(new Error("400 ::: Invalid sort direction"));
});
...
test(`Return data when sort_direction pass with A `, async () => {
	...
	expect(await approvedDomainGetAll(data)).toBeNull();
...
test(`Error when query return exception`, async () => {
	...
	expect(await approvedDomainGetAll(data)).toEqual(new Error("400 ::: Unknown error"));
});
...
test(`Error when user pass wrong query params`, async () => {
	const data = getValidRequest();
	data.query = 123;
	expect(await approvedDomainGetAll(data, ctx)).toEqual(new Error(`400 ::: Query invalid`));
});
...
test(`Invalid offset value as "123" provided`, async () => {
	const data = getValidRequest();
	data.limit = "123";
	expect(await approvedDomainGetAll(data, ctx)).toEqual(new Error(`400 ::: Limit invalid`));
...
test(`Invalid offset value as -1 provided`, async () => {
	const data = getValidRequest();
	data.limit = -1;
	expect(await approvedDomainGetAll(data, ctx)).toEqual(new Error(`400 ::: Limit must be positive`));
	expect(ctx.body).toBeNull();
});
...
test(`Invalid offset value as "123" provided`, async () => {
	const data = getValidRequest();
	data.offset = "123";
	expect(await approvedDomainGetAll(data, ctx)).toEqual(new Error(`400 ::: Offset invalid`));
	expect(ctx.body).toBeNull();
});
...
test(`Invalid offset value as -1 provided`, async () => {
	const data = getValidRequest();
	data.offset = -1;
	expect(await approvedDomainGetAll(data, ctx)).toEqual(new Error(`400 ::: Offset must not be negative`));
	expect(ctx.body).toBeNull();
});
