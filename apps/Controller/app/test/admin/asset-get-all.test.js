const assetGetAllRaw = require("../../core/admin/asset-get-all");
...
async function assetGetAll(data) {
	let err = null;
	try {
		ctx.body = await assetGetAllRaw(data, ctx);
	} catch (e) {
		err = e;
	}
	return err;
...
test(`Error when user role is not cla-admin`, async () => {
	...
	expect(await assetGetAll(data)).toEqual(new Error("401 ::: Unauthorized"));
});
...
test(`Error when limit is invalid`, async () => {
	...
	const data = getValidRequest();
	data.limit = "test";
	expect(await assetGetAll(data)).toEqual(new Error("400 ::: Limit invalid"));
});
...
test(`Error when limit is negative`, async () => {
	...
	const data = getValidRequest();
	data.limit = -40;
	expect(await assetGetAll(data)).toEqual(new Error("400 ::: Limit must be positive"));
});
...
test(`Error when offset is invalid`, async () => {
	...
	const data = getValidRequest();
	data.offset = "test";
	expect(await assetGetAll(data)).toEqual(new Error("400 ::: Offset invalid"));
});
...
test(`Error when offset is negative`, async () => {
	...
	const data = getValidRequest();
	data.offset = -40;
	expect(await assetGetAll(data)).toEqual(new Error("400 ::: Offset must not be negative"));
});
...
test(`Error when sort_direction not pass`, async () => {
	...
	expect(await assetGetAll(data)).toEqual(new Error("400 ::: Sort Direction not provided"));
});
...
test(`Error when sort_direction is positive integer number`, async () => {
	...
	expect(await assetGetAll(data)).toEqual(new Error("400 ::: Sort Direction invalid"));
});
...
test(`Error when sort_direction is invalid`, async () => {
	...
	expect(await assetGetAll(data)).toEqual(new Error("400 ::: Invalid sort direction"));
});
...
test(`Error when sort_field not pass`, async () => {
	...
	expect(await assetGetAll(data)).toEqual(new Error("400 ::: Sort field not found"));
});
...
test(`Error when query is invalid`, async () => {
	...
	const data = getValidRequest();
	data.query = 123;
	expect(await assetGetAll(data)).toEqual(new Error("400 ::: Query invalid"));
});
...
test(`Unknown Error while executing the count query`, async () => {
	...
	const data = getValidRequest();
	expect(await assetGetAll(data)).toEqual(new Error("400 ::: Unknown error"));
});
...
test(`Success while executing the result query`, async () => {
	...
	const data = getValidRequest();
	expect(await assetGetAll(data)).toBeNull();
