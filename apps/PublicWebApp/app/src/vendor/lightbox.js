...
var ReactImageLightbox =
	...
	(function (_Component) {
		...
		_createClass(ReactImageLightbox, null, [
			{
				...
				value: function isTargetMatchImage(target) {
					return target && !!target.closest(".ril-image-current");
				},
			},
			{
				key: "parseMouseEvent",
			...
			},
			{
				...
				value: function getTransform(_ref) {
					...
					const transformStyle = "translate3d("
						...
						.concat(" rotate(", _ref.rotateDegree + "deg)");
					return {
		...
		function ReactImageLightbox(props) {
			...
			//access the method when rotate
			_this.changeZoom = _this.changeZoom.bind(_assertThisInitialized(_assertThisInitialized(_this)));
			return _this;
		}
		...
		_createClass(ReactImageLightbox, [
			...
			},
			{
				...
				value: function componentDidMount() {
					...
					//TVF CLA Commented For drag and resize the notes
					this.listeners = {
			...
			},
			{
				...
				value: function UNSAFE_componentWillReceiveProps(nextProps) {
					...
					//update the flag when rotate trigger
					if (_this3.props.rotateDegree !== nextProps.rotateDegree) {
						this.setState({
							isRotate: true,
						});
					}
			...
			},
			{
				...
				value: function handleKeyInput(event) {
					//TVF CLA for allow user to edit the content with press left and right arrow key in full screen
					//class list
					if (event.target.className.indexOf("ril-outer") === -1) {
						return;
					}
			...
			},
			{
				...
				value: function shouldHandleEvent(source, event) {
			...
			},
			{
				...
				value: function handleMouseDown(event) {
					if (this.shouldHandleEvent(SOURCE_MOUSE, event) && ReactImageLightbox.isTargetMatchImage(event.target)) {
			...
			},
			{
				...
				value: function handleMouseMove(event) {
					if (this.shouldHandleEvent(SOURCE_MOUSE, event)) {
			...
			},
			{
				...
				value: function handlePointerEvent(event) {
					if (this.shouldHandleEvent(SOURCE_POINTER, event)) {
						switch (event.type) {
							case "pointerdown":
			...
			},
			{
				...
				value: function handleTouchStart(event) {
					...
					if (this.shouldHandleEvent(SOURCE_TOUCH, event) && ReactImageLightbox.isTargetMatchImage(event.target)) {
			...
			},
			{
				...
				value: function handleTouchMove(event) {
					if (this.shouldHandleEvent(SOURCE_TOUCH, event)) {
				...
				value: function multiPointerStart(event) {
					...
					switch (this.pointerList.length) {
						case 1: {
							/**
							 * CLA EP
							 * Commented out because this was stopping us from using mousedown
							 * event listeners on any element that overlapped the image.
							 * This might break the lightbox swiping/pinching functionality though...
							 */
							// event.preventDefault();
				...
				value: function render() {
					...
					var addImage = function addImage(srcType, imageClass, transforms) {
						...
						var imageStyle = _objectSpread(
							{},
							transitionStyle,
							ReactImageLightbox.getTransform(_objectSpread({}, transforms, bestImageInfo, _this16.props))
						);
						if (isRotate) {
							_this16.changeZoom(0, offsetX, offsetY);
						}
						...
						if (discourageDownloads) {
							...
							const imageAlt = typeof imageTitle === "string" ? imageTitle : translate("Image");
							const key = imageSrc + keyEndings[srcType];
							const className = "".concat(imageClass, " ril__image");
							const opts = {
								onDoubleClick: _this16.handleImageDoubleClick,
								onWheel: _this16.handleImageMouseWheel,
							...
							const isCurrent = !!className.match(/\bril-image-current\b/);
							if (isCurrent && _this16.props.currentImageRef) {
								opts.ref = _this16.props.currentImageRef;
							}
							const finalImageOpts = _extends(
								{},
								imageCrossOrigin
									? {
							...
							const imageElement = React.createElement("img", finalImageOpts);
							if (_this16.props.ImageWrapComponent) {
								const ImageWrapComponent = _this16.props.ImageWrapComponent;
								images.push(
									<ImageWrapComponent
										image_props={finalImageOpts}
										key={key}
										page_data={_this16.props.image_wrap_component_page_data}
					...
					return React.createElement(
						...
						React.createElement(
										...
										React.createElement(
											...
											React.createElement("button", {
												...
												className: ["ril-zoom-in", "ril__toolbarItemChild", "ril__builtinButton", "ril__zoomInButton"]
													.concat(_toConsumableArray(zoomLevel === MAX_ZOOM_LEVEL ? ["ril__builtinButtonDisabled"] : []))
													.join(" "),
												ref: this.zoomInBtn,
										...
										React.createElement(
											...
											React.createElement("button", {
												...
												className: ["ril-zoom-out", "ril__toolbarItemChild", "ril__builtinButton", "ril__zoomOutButton"]
													.concat(_toConsumableArray(zoomLevel === MIN_ZOOM_LEVEL ? ["ril__builtinButtonDisabled"] : []))
													.join(" "),
												ref: this.zoomOutBtn,
							...
							this.props.imageCaption && // eslint-disable-next-line
								React.createElement(
									"div",
									{
										// Image caption
