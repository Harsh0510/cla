					...
					.then((result) => {
						...
						if (result.result) {
							this.setState({
								message: (
									<div>
										<h4>Your email address was verified successfully.</h4> You will not be able to log in until your institution administrator has
										approved your registration request.
									</div>
								),
								messageType: messageType.success,
							});
						} else {
							this.setState({ message: `Something went wrong. Please try again later.`, messageType: messageType.error });
						}
					})
					.catch((result) => {
						if (!this._isMounted) {
												...
												{show ? (
													<>
														...
														<FormContainerButton>
															...
															{/* <Button onClick={this.handleSubmit} name="btnRegister"> Register </Button> */}
														</FormContainerButton>
													</>
												) : (
													""
