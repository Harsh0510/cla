...
import ApprovedDomainAddEdit from "./ApprovedDomainAddEdit";
...
export default withAdminAuthRequiredConsumer(
	{ "cla-admin": true },
	withApiConsumer(
		class ApprovedDomains extends React.PureComponent {
			constructor(props) {
				super(props);
				this.state = {
					limit: 10,
					offset: 0,
					sort_field: "school_name",
					sort_dir: "asc",
					query: "",
					loading: true,
					isLoaded: false,
			...
			updateState() {
				...
				let sortField = parsed.sort_field || "school_name";
			...
			performQuery() {
				this.props
					.api("/admin/approved-domain-get-all", {
					...
					.then((result) => {
						...
						if (this.state.action == ACTION_NEW || this.state.action == ACTION_ADDED) {
							...
							fields.school_id = selectedRowData.school_id;
							fields.school_name = selectedRowData.school_name;
			...
			getData = (data, unfiltered_count) => {
				//declare columns
				let columns = [
					{ name: "school_name", title: "Institution Name" },
				...
				let defaultColumnWidths = [
					{ columnName: "school_name", width: 700 },
				...
				let tableColumnExtensions = [
					{ columnName: "school_name", align: COLUMN_ALIGN_LEFT },
			 ...
			 */
			pushHistory(extra) {
				const url = "/profile/admin/approved-domains?" + this.getQueryString(extra);
			...
			handleSubmit = (data) => {
				if (this.state.action == ACTION_NEW || this.state.action == ACTION_ADDED) {
					const params = {
						school_id: parseInt(data.school, 10),
						domain: data.domain,
					};

					/**Removed below commented code when end-point is ready to use */
					this.props
						.api("/admin/approved-domain-create", params)
						.then((result) => {
							if (result.success) {
								this.setState({ message: "Approved Domain added successfully" });
					...
					this.props
						.api("/admin/approved-domain-update", params)
						.then((result) => {
							if (result) {
								this.setState({ message: "Approved Domain updated successfully" });
								this.performQuery();
							}
						})
						.catch((result) => {
			...
			getFieldValuesForUpdate = (currentRowDBData, updatedRowDetail) => {
				...
				if (currentRowDBData.school_id != updatedRowDetail.school) {
					params.school_id = parseInt(updatedRowDetail.school, 10);
				}
			...
			deleteData = () => {
				this.props
					.api("/admin/approved-domain-delete", { id: parseInt(this.state.oid, 10) })
					.then((result) => {
						if (result.result) {
							this.setState({ message: "Approved Domain deleted successfully" });
			...
			render() {
				...
				return (
					<>
						<HeadTitle title={PageTitle.approvedDomains} />
						<Header jumpToContentId={JUMP_TO_CONTENT_ID} />
						<AdminPageWrap pageTitle={"Approved Domains"} id={JUMP_TO_CONTENT_ID}>
							<PageDetail>
								<SearchSectionOne>
									<FilterSectionHalf>
										<FilterSearchBar
									...
									<SectionHalf>
										<Button
											...
											onClick={this.createApprovedDomain}
