...
import ApprovedDomains from "../index";
import MockApprovedDomains from "../../../mocks/MockApprovedDomains";
...
let location, sortingA, sortingD, page, limit, history, mockUserData, mockApprovedDomainsData;
...
async function defaultApi(endpoint, data) {
	// endpoints
	if (endpoint === "/admin/approved-domain-get-all") {
		if (data) {
			return mockApprovedDomainsData;
		} else {
			return {
				data: [],
				unfiltered_count: 0,
...
function resetAll() {
	...
	mockApprovedDomainsData = MockApprovedDomains;
}
...
test("Component renders correctly", async () => {
	const item = shallow(<ApprovedDomains location={location} api={defaultApi} withAuthConsumer_myUserDetails={mockUserData} />);
...
test("Component renders correctly without blank data", async () => {
	...
	const item = shallow(<ApprovedDomains location={location} api={api} withAuthConsumer_myUserDetails={mockUserData} />);
...
test("When user click on Search button and do serching", async () => {
	const item = shallow(<ApprovedDomains location={location} api={defaultApi} history={history} withAuthConsumer_myUserDetails={mockUserData} />);
...
test("User click on create button", async () => {
	...
	item.instance().createApprovedDomain();
	const push = item.instance().props.history.push;
	await wait(100);
	item.update();
	item.instance().forceUpdate();
	expect(push.mock.calls[0][0]).toEqual("/profile/admin/approved-domains?action=new&limit=10&offset=0&oid&query&sort_dir=asc&sort_field=school_name");
});
...
test("User click on cancel button", async () => {
	...
	expect(push.mock.calls[0][0]).toEqual(
		"/profile/admin/approved-domains?action=" +
			ACTION_LIST +
			"&limit=" +
			item.state().limit +
			"&offset=" +
			item.state().offset +
			"&oid&query&sort_dir=asc&sort_field=school_name"
	);
});

/** User click on delete Approved Domain data */
test("User click on delete button", async () => {
	async function api(endpoint, data) {
		if (endpoint === "/admin/approved-domain-delete") {
			return {
				result: true,
			};
		}
	...
	const item = shallow(<ApprovedDomains location={location} api={api} history={history} withAuthConsumer_myUserDetails={mockUserData} />);
	...
	expect(item.state().message).toEqual("Approved Domain deleted successfully");
});
...
/** User click on submit for create Approved Domain */
test("User click on submit for create but return exception", async () => {
	...
	async function api(endpoint, data) {
		if (endpoint === "/admin/approved-domain-create") {
			throw "Unknown error";
		}
		return defaultApi(endpoint, true);
	}
...
/** User click on submit for create Approved Domain */
test("User click on submit for create Approved Domain", async () => {
	location.search = "?action=" + ACTION_NEW + "&limit=10&offset=0&oid&sort_dir=A&sort_field=title";
	async function api(endpoint, data) {
		if (endpoint === "/admin/approved-domain-create") {
			if (data !== "") {
	...
	item.update();
	item.instance().forceUpdate();
	expect(item.state().message).toEqual("Approved Domain added successfully");
});
...
test("User clicks submit for update domain details", async () => {
	...
	async function api(endpoint, data) {
		if (endpoint === "/admin/approved-domain-update") {
			return {
				result: true,
			};
		}
	...
	expect(item.state().message).toEqual("Approved Domain updated successfully");
});
...
test("User click on pagination page", async () => {
	...
	let mockurl = "/profile/admin/approved-domains?action=list&limit=10&offset=" + setOffset + "&oid&query&sort_dir=asc&sort_field=school_name";
	await wait(50);
...
test("User pass invalid limit and offset value", async () => {
	location.search = "?action=" + ACTION_LIST + "&limit=0&offset=-10&oid&sort_dir=d&sort_field=school_name";
	async function api(endpoint, data) {
		if (endpoint === "/admin/approved-domain-get-all") {
			if (data !== "") {
				return {
...
test("User click on Edit Icon from list", async () => {
	...
	expect(push.mock.calls[0][0]).toEqual(
		"/profile/admin/approved-domains?action=edit&limit=" +
			item.state().limit +
			"&offset=" +
			item.state().offset +
			"&oid=" +
