...
let fields, message, examBoards, ACTION_NEW, ACTION_EDIT, CONFIRM_DIALOG_DELETE, CONFIRM_DIALOG_NONE, mockFormData, mockUserData;
...
test("User click on create button from above the listing page, It should display the create button in form", async () => {
	...
	expect(item.find('button[name="create-ApprovedDomain"]').length).toBe(1);
});
...
test("User click on edit icon from the listing page, It should display the update button in form", async () => {
	...
	expect(item.find('button[name="update-ApprovedDomain"]').length).toBe(1);
});
...
test("User click on submit for create a new domain", async () => {
	...
	form.simulate("submit", {
		...
		target: [mockFormData],
		relatedTarget: { value: "create-ApprovedDomain" },
	});
...
test("User click on submit for update domain details", async () => {
	...
	form.simulate("submit", {
		preventDefault: mockCall,
		target: [mockFormData],
		relatedTarget: { value: "update-ApprovedDomain" },
	});
...
test("User click on submit get message", async () => {
	...
	item.setProps({ message: "Approved Domain updated successfully" });
