...
export default class ApprovedDomainAddEdit extends React.PureComponent {
	constructor(props) {
		super(props);
		this.state = {
	...
	render() {
		...
		return (
			<FormWrapAddEdit onSubmit={this.doSubmit}>
				...
				<FormBodyContainer>
					<FormContainerFull>
						<FormContainerHalf marginBottom="0px">{this.schoolsDropdown()}</FormContainerHalf>
					...
					<FormContainerButton>
						{action === ACTION_NEW || action === ACTION_ADDED ? (
							<FormSaveButton
								type="submit"
								name="create-ApprovedDomain"
								value="create-ApprovedDomain"
							...
							<>
								<FormSaveButton
									type="submit"
									hide={!!confirmBox}
									name="update-ApprovedDomain"
									value="update-ApprovedDomain"
									disabled={this.state.domain_field_error ? true : false}
								>
									Update
								</FormSaveButton>
