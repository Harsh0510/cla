...
export default withAdminAuthRequiredConsumer(
	{ "cla-admin": true },
	withApiConsumer(
		class AdminAssetCrudPage extends React.PureComponent {
			constructor(props) {
				super(props);
				this.state = {
					limit: 10,
				...
				this.props
					...
					.then((result) => {
						...
						if (
							...
							fields.isbn13 = itemDetails.isbn13;
							fields.pdf_isbn13 = itemDetails.pdf_isbn13;
							fields.publisher_name_log = itemDetails.publisher_name_log;
							fields.imprint = itemDetails.imprint;
			...
			getFieldValuesForUpdate = (currentItem, updatedItem) => {
				...
				if (currentItem.active !== updatedItem.active) {
					params.active = !!updatedItem.active;
				}
			 ...
			 */
			pushHistory(extra) {
				const url = "/profile/admin/assets?" + this.getQueryString(extra);
			...
			render() {
				...
				return (
					<>
						<HeadTitle title={PageTitle.adminAssetCrud} />
						<Header jumpToContentId={JUMP_TO_CONTENT_ID} />
						<AdminPageWrap pageTitle={"Assets"} id={JUMP_TO_CONTENT_ID}>
							<PageDetail>
								<SearchSectionOne>
									<FilterSectionHalf>
										<FilterSearchBar
