...
import MockAssetAll from "../../../mocks/MockAssetAll";
import AdminAssetCrud from "../index";
...
let location, sortingA, sortingD, page, limit, history, mockUserData, mockAssetData;
...
async function defaultApi(endpoint, data) {
	// endpoints
	if (endpoint === "/admin/asset-get-all") {
		if (data) {
...
function resetAll() {
	...
	(mockUserData.role = "cla-admin"), (mockAssetData = MockAssetAll);
}
...
test("Component renders correctly", async () => {
	const item = shallow(<AdminAssetCrud location={location} api={defaultApi} withAuthConsumer_myUserDetails={mockUserData} />);
...
test("When user click on Search button and do serching", async () => {
	const item = shallow(
		<AdminAssetCrud location={location} api={defaultApi} history={history} hide={true} withAuthConsumer_myUserDetails={mockUserData} />
	);
...
test("When user closes Add/Edit item form", async () => {
	...
	const mockurl = "/profile/admin/assets?action=list&id&limit=10&offset=0&query&sort_dir=A&sort_field=title";
...
test("When user click on Edit button in asset", async () => {
	//mock oid
	let oID = mockAssetData.data[0].id;
...
test("When user click on submit button in edit section", async () => {
	...
	async function api(endpoint, data) {
		if (endpoint === "/admin/asset-update") {
			return { result: true };
		}
		return defaultApi(endpoint, true);
	}

	const item = shallow(<AdminAssetCrud location={location} api={api} history={history} withAuthConsumer_myUserDetails={mockUserData} />);
...
test("When user click on cancel button while Edit Asset Page", async () => {
	const item = shallow(<AdminAssetCrud location={location} api={defaultApi} history={history} withAuthConsumer_myUserDetails={mockUserData} />);
...
test("When user click on pagination page", async () => {
	const item = shallow(
		<AdminAssetCrud location={location} history={history} api={defaultApi} onPageChanged={jest.fn()} withAuthConsumer_myUserDetails={mockUserData} />
	);
	...
	let mockurl = "/profile/admin/assets?action=list&id&limit=10&offset=" + setOffset + "&query&sort_dir=A&sort_field=title";
	await wait(20);
...
test("When User search in search text box", async () => {
	const item = shallow(<AdminAssetCrud location={location} api={defaultApi} withAuthConsumer_myUserDetails={mockUserData} history={history} />);
	await wait(50);
...
test("When user click on column to perform ascending sorting action", async () => {
	...
	const item = shallow(<AdminAssetCrud location={location} history={history} api={defaultApi} withAuthConsumer_myUserDetails={mockUserData} />);
	...
	let mockurl = "/profile/admin/assets?action=list&id&limit=10&loading=true&offset=" + item.state().offset + "&query&sort_dir=A&sort_field=title";
	expect(push.mock.calls[0][0]).toEqual(mockurl);
});
...
test("When user click on column to perform descending sorting action", async () => {
	...
	let mockurl = "/profile/admin/assets?action=list&id&limit=10&loading=true&offset=" + item.state().offset + "&query&sort_dir=D&sort_field=title";
	expect(push.mock.calls[0][0]).toEqual(mockurl);
});
