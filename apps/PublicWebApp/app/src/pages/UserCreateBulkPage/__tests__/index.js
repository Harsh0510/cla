...
import UserCreateBulkPage from "../index";
import Header from "../../../widgets/Header";
import MockUserRole from "../../../mocks/MockUserRole";
import MockUser from "../../../mocks/MockUser";
import MockSchoolList from "../../../mocks/mockSchoolList";
import { MockFileJsonData, MockBulkUploadSuccessResult, MockBulkUploadErrorsResult, MockBulkUploadResult } from "../../../mocks/mockUserBulkData";

let props, mockUserData, file, message, mockCell, select_school, valid, mockResultFileJsonData;
let mockIsCalledDownloadFile = false;
...
function resetAll() {
	...
	(valid = {
		...
		(mockResultFileJsonData = MockFileJsonData);
...
test(`Component renders correctly`, async () => {
	const item = shallow(<UserCreateBulkPage api={defaultApi} withAuthConsumer_myUserDetails={mockUserData} {...props} />);
...
test(`User upload the file and click on submit button for \`Bulk Create User\``, async () => {
	const item = shallow(<UserCreateBulkPage api={defaultApi} withAuthConsumer_myUserDetails={mockUserData} {...props} />);

	item.instance().handleUpload(file);
	const spy = jest.spyOn(item.instance(), "createBulkUsers");
	item.update();
	await wait(50);
	item.instance().handleSubmit({ preventDefault: jest.fn() });
	await wait(50);
...
test(`Error Spreadsheet must have at least one row When upload file dont have Array data`, async () => {
	...
	expect(item.state().message).toEqual("Spreadsheet must have at least one row");
...
test(`Component renders correctly with cla-admin`, async () => {
	...
	expect(item.find("CreateBulkUsersForm").children().find("CustomSelectSchool").length).toBe(1);
});
...
test(`Component renders correctly with school-admin`, async () => {
	...
	expect(item.find("CreateBulkUsersForm").children().find("SelectSearch").length).toBe(0);
});
...
test(`User upload the file and click on submit button`, async () => {
	async function api(endpoint, data) {
		if (endpoint === "/auth/user-create-bulk") {
			return MockBulkUploadResult;
		}
		return defaultApi(endpoint, data);
	}
	const item = shallow(<UserCreateBulkPage api={api} withAuthConsumer_myUserDetails={mockUserData} {...props} />);
...
test(`User upload the file and click on downloadAReport button`, async () => {
	...
	item.update();
	item.instance().forceUpdate();
	const downloadAReport = item.instance().doDownloadTemplate({ preventDefault: mockPreventDefault });
...
test("User Upload file and get download message", async () => {
	async function api(endpoint, data) {
		if (endpoint === "/auth/user-create-bulk") {
			return { results: [{ success: false, message: "A user with that email already exists", httpcode: 400 }] };
		}
		return defaultApi(endpoint, data);
	}
	const users = [
		{ last_name: "TestLast", first_name: "testFirst", title: "Mr", job_title: "teacher", email: "test@email.com" },
		{ last_name: "TestLname", first_name: "TestFname", title: "Mr", job_title: "teacher", email: "test2@email.com" },
	];
	const item = shallow(<UserCreateBulkPage api={api} withAuthConsumer_myUserDetails={mockUserData} {...props} />);

	item.instance().createBulkUsers(users);
	await wait(50);
	item.update();
	expect(item.state().errorsData.length).toBe(1);
	expect(item.state().successData.length).toBe(0);
