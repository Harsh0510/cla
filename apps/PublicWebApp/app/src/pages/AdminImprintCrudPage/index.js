...
export default withAdminAuthRequiredConsumer(
	{ "cla-admin": true },
	withApiConsumer(
		class AdminImprintCrudPage extends React.PureComponent {
			constructor(props) {
				super(props);
				this.state = {
					limit: 10,
				...
				this.props
					...
					.then((result) => {
						...
						if (
							...
							fields.external_identifier = itemDetails.external_identifie || "";
			...
			getItem = (itemData, unfiltered_count) => {
				...
				const defaultColumnWidths = [
					...
					{
						columnName: "external_identifier",
						width: 200,
					},
					{
			...
			handleSubmit = (data) => {
				if (this.state.action == ACTION_EDIT && this.state.currentItem) {
					...
					this.props
						.api("/admin/imprint-update", params)
						.then((result) => {
							if (result.result) {
			 ...
			 */
			pushHistory(extra) {
				const url = "/profile/admin/imprints?" + this.getQueryString(extra);
			...
			render() {
				...
				return (
					<>
						<HeadTitle title={PageTitle.adminImprintCrud} />
						<Header jumpToContentId={JUMP_TO_CONTENT_ID} />
						<AdminPageWrap pageTitle={"Imprints"} id={JUMP_TO_CONTENT_ID}>
							<PageDetail>
								<SearchSectionOne>
									<FilterSectionHalf>
										<FilterSearchBar
